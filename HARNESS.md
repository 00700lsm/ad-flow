...
Red → Green → Lint → Refactor → Summary
        ...
        ↓
개발자: 다음  /  커밋 푸시  /  화면 피드백
에이전트: Summary면 그 Task 커밋 (ADR 014). 푸시 없음
```
...
Summary가 끝나면 에이전트가 그 Task만 커밋한다 (ADR 014). 사람이 `커밋`을 다시 치지 않아도 된다.
...
Plan 중에는 커밋하지 않는다. 재현 커밋과 해법 커밋을 섞지 않는다. 푸시는 요청할 때만.
